...
size_t BinToUf2(const std::string &BinFile, const std::string &Uf2File,uint32_t FamilyID , uint32_t AppstartAddr) {
...
std::string BinToUf2(const std::string &BinData,uint32_t FamilyID , uint32_t AppstartAddr) {
  auto UF2_MAGIC_START0 = 0x0A324655; // "UF2\n"
  auto UF2_MAGIC_START1 = 0x9E5D5157; // Randomly selected
  auto UF2_MAGIC_END = 0x0AB16F30; // Ditto
  auto file_content = std::string(BinData.begin(), BinData.end());
  std::vector<uint8_t> datapadding;
  while (datapadding.size() < 512 - 256 - 32 - 4) {
    datapadding.push_back(0);
    datapadding.push_back(0);
  ...
  std:: string ret(outp.begin(),outp.end());
  return std::move(ret);
}
