...
typedef void (*PluginButtonPinFunc)(int);
...
void CallPluginButtonPin(const QString &type, ButtonPinCallType pin) {
  if (pin<1|| pin>4) {
    throw std::runtime_error("无效的按钮事件");
  }
  static std::vector arrs({"左侧按钮短按事件", "右侧按钮短按事件", "左侧按钮长按事件", "右侧按钮长按事件"});
  PrintDebug("扩展通知: {}", arrs[pin - 1]);
  for (auto &[name, pluginInfo]: g_plugins[type]) {
    CallPluginButtonPin(type, name, pin);
  }
}
...
bool CallPluginButtonPin(const QString &type, const QString &name, ButtonPinCallType pin) {
  ...
  PluginButtonPinFunc callFunc = nullptr;
  if (plugin.CallFuncsTable.find("PluginButtonPin") == plugin.CallFuncsTable.end()) {
    auto ptr = (PluginButtonPinFunc) GetPluginFunc(plugin.Handle, "PluginButtonPin");
    if (ptr == nullptr) {
      throw std::runtime_error("获取插件函数失败");
    }
    callFunc = ptr;
  } else {
    callFunc = (PluginButtonPinFunc) plugin.CallFuncsTable["PluginButtonPin"];
  }
  if (callFunc != nullptr) {
    callFunc(int(pin));
  }
